...
{
    HILOG_INFO("CallLogCheckAbility OnConnect begin");
    ...
    HILOG_INFO("CallLogCheckAbility OnConnect end");
...
{
    HILOG_INFO("CallLogCheckAbility OnStart begin");
...
void CallLogCheckAbility::AddQueryNotPrivacyCondition(OHOS::NativeRdb::RdbPredicates &rdbPredicates)
{
    ...
    if (whereSql.find(Contacts::CallLogColumns::PRIVACY_TAG) == std::string::npos) {
        HILOG_INFO("query calllog, not has privacy_tag conditoin, add condition");
