    ...
    zipFile& zipfile, const std::string& srcFile, int keepParentPathStatus, const std::string& destFileName)
{
    ...
    if (result != EOK) {
        HILOG_ERROR("AddFileInZip memset_s error, file:%{public}s.", srcFile.c_str());
        return ERROR_MEMSET_STRUCT;
    }
    FILE *srcFp = GetFileHandle(srcFile);
    if (srcFp == nullptr) {
        HILOG_ERROR("get file handle failed:%{public}s, errno: %{public}d.", srcFile.c_str(), errno);
    ...
    while (!feof(srcFp)) {
        ...
        if (ferror(srcFp)) {
            HILOG_ERROR("zip file failed:%{public}s, errno: %{public}d.", srcFile.c_str(), errno);
            errcode = errno;
            break;
        }
    }
